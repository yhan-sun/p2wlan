...
大规模部署中，全局拓扑按稳定账号 ID 游标分批读取，并受明确的节点/边预算保护。响应会返回已加载账号数、是否完整以及预算不足原因；达到预算时管理台显示“不完整”并要求下钻到具体账号，而不是静默截断后仍声称全局图完整。账号和设备列表均使用稳定 ID 游标，因此设备心跳改变 `last_seen` 时不会改变其分页顺序。设备列表的 `/admin/api/v1/devices/cursor` 游标绑定搜索词和在线状态，改变筛选时从首页重新读取；每页返回当前事务快照及 `generated_at`，不会冻结整个浏览会话。新增设备或状态变化落在已翻过的 ID 范围时，返回首页可查看当前结果。网络和房间列表按页读取，不在打开页面时一次扫描全部记录。
...
小时 rollup 每个 network 每小时只有一行并保留 720 小时；不会长期保存 peer/device 级事件明细。管理台的连接健康页提供最近 24 小时、7 天或 30 天的趋势图，可随网络筛选查看路径观测样本、切换与显式失败，以及平均 RTT 和 P95 分桶上界；小时明细支持键盘操作。没有 RTT 样本的小时保持空缺，P95 超出最高分桶时明确标识为大于 10000 ms，不绘制虚假的零值。账号或设备范围会应用于实时健康数据；由于历史数据仅按网络汇总，启用这些范围时会暂停展示趋势，并提供保留网络筛选、清除账号和设备范围的入口。

### 刷新和排障导航

管理台顶部可切换自动刷新，也可随时手动刷新。资源列表、账号详情、关系、连接与健康页面显示最后成功更新时间；离线或后台刷新失败时保留已有快照并明确标注，缓存中的在线或新鲜标记不代表此刻仍可确认的状态。网络和房间分别加载，单个页签失败不会阻止另一页签使用。

设备可跳到所属账号、资源关系与关联连接，账号和网络也提供连接入口。搜索、作用域、页码、视图与连接详情方向保存在页面 URL 中，支持刷新、浏览器返回和分享定位；URL 不包含管理员令牌。游标页的向前记录保留在有界的浏览器历史状态中，分享的独立深链若无历史记录，则提供返回首页入口。
