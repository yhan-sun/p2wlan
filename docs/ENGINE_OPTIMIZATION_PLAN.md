...
- 多 STUN 观察点候选收集、行为型 `NatProfile` 本地诊断、受限端口预测 candidate 广告，以及 `p2wlan doctor` NAT/STUN 建议输出（`NAT-01a`/`NAT-05a` 已落地；仍保持旧控制面 candidate wire format）。
...
- 预测 candidate 仍走现有控制面 candidate 字符串，不改变 wire format；实际可用性仍必须通过认证 probe / candidate pair 状态机确认。
- `NatProfile.predicted_endpoints` 和 `p2wlan doctor` 的 `predicted=N` 用于实机验收，便于对比 prediction 在家庭路由、热点、云服务器 NAT 下的真实收益。

后续 `NAT-01c` 再把这些本地匿名聚合结果接入 prediction / birthday budget 决策；默认不上传公网 IP、SSID、BSSID、路由器型号等识别性信息。
