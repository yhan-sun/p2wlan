...
- 多 STUN 观察点候选收集、行为型 `NatProfile` 本地诊断、受限端口预测 candidate 广告、candidate source sidecar，以及 `p2wlan doctor` NAT/STUN 建议输出（`NAT-01a`/`NAT-05a`/`NAT-05b` 已落地；仍保持旧控制面 endpoint 字符串兼容）。
...
- 控制面签发短期 Relay token，至少绑定 `device_id`、`credential_id`、`network_id`、Relay audience、过期时间和随机 ID。
...
Relay token 当前使用控制面 Ed25519 签名 JWT；关键边界是短期、可验证、可撤销、绑定 `credential_id` 并限定 audience。Relay 可合并本地静态 denylist 和控制面 `GET /api/v1/relay/revocations` 在线 snapshot；feed 拉取失败时保留上一份成功 snapshot，不放开已知撤销项。
...
- 定义设备删除、凭据撤销和密钥轮换后的连接终止行为；当前已保留 relay revocation tombstone，避免 device 删除后因 cascade 丢失 feed 所需 ID。
...
`NAT-01a` 不改变控制面 candidate wire format。认证 Probe v2 skeleton 已允许 daemon 在 MAC 验证后学习 peer-reflexive endpoint；prediction、birthday probing、filtering 行为探测和预算调度仍由后续 NAT-01b/Phase E 承接。

当前已落地的 `NAT-01b-a` 是行为诊断基础，不主动增加 socket：

- `NatProfile` 增加 `filtering_behavior`、`hairpin_behavior`、`mapping_lifetime` 占位，保持 diagnostics JSON 向后兼容。
- 根据多 STUN mapping 结果推断 `prediction_candidate` 和 `birthday_candidate`，用于后续 probe 预算、端口预测和 birthday probing 决策。
- `p2wlan doctor` 输出 filtering/hairpin/lifetime/prediction/birthday 摘要，并在可预测端口 delta 或 birthday 候选网络下给出明确建议。

当前已落地的 `NAT-01b-b` 是有预算的主动本地探测，仍复用数据面的同一个 UDP socket：

- 对第一个成功 STUN observer 发送短超时 CHANGE-REQUEST；只有响应源地址/端口确实变化时才把 filtering 升级为强判定，避免把不支持 RFC 5780 的普通 STUN 服务器误判为 NAT 行为。
- 对已发现的公网 endpoint 发送自地址 hairpin probe，短窗口内收到原始 payload 则标记 `hairpin_behavior=supported`，超时则标记 `unsupported`。
- 普通 STUN 成功后等待一个短 idle 窗口再重新查询同一 observer；映射保持一致则记录 `mapping_lifetime=lower_bound_ms(...)`，作为后续预算和预测策略的弱信号。

当前已落地的 `NAT-01b-c` 是真实 candidate pair 成功率回写：

- 每个 pair 的成功/失败样本继续保留在明细 diagnostics 中，同时按 endpoint source 聚合为 `candidate_pair_stats`，输出成功率、当前 generation pair 数、状态计数和最近成功/失败年龄。
- probe 排序会读取同一份 source-level 成功率，在 pair 状态相同的情况下优先尝试历史真实成功率更高的来源，而不是只依赖静态 source 优先级。
- `p2wlan doctor` 输出 `pair-stats` 摘要，便于实机对比 signaled、learned、peer-reflexive 的真实效果。

当前已落地的 `NAT-05a` 是受限端口 prediction 的第一步：

- 仅当 `NatProfile.prediction_candidate=true`、公网 IP 稳定、公网端口不稳定且连续端口 delta 落在小范围内时，才生成预测 candidate。
- 预测从最后一次成功 STUN 观测端口继续外推，最多广告 4 个额外 server-reflexive endpoint，避免把已经观测过的端口重复加入候选集。
- `NatProfile.predicted_endpoints` 和 `p2wlan doctor` 的 `predicted=N` 用于实机验收，便于对比 prediction 在家庭路由、热点、云服务器 NAT 下的真实收益。

当前已落地的 `NAT-05b` / `NAT-04a` 把 prediction 纳入可观测、可限流的 candidate-pair 决策：

- `IceCandidate` 增加内部 `source` 元数据；控制面继续保留 `candidates: Vec<String>` endpoint 字符串，并通过可选 `candidate_sources` sidecar 传递新客户端可理解的来源。
- daemon 接收 candidate 时把 `predicted` 记录为独立 `CandidatePairSource::Predicted`，旧客户端或缺失 sidecar 的候选仍按 `signaled` 处理。
- probe scheduler 对 predicted pair 设置独立预算：没有成功历史时每轮最多探测 2 个 predicted endpoint；已有成功样本后再按 source 成功率排序放开。
- `candidate_pair_stats` 和 `p2wlan doctor` 会输出 `predicted=x/y:rate,current=n`，用于真实网络 A/B 验证。

当前已落地的 `NAT-03a` / `NAT-04b` 把运行期反馈升级为本地匿名历史和动态预算：

- daemon 在配置文件同目录维护 `traversal-history.json`，只保存 candidate source 级别的 success/failure、连续失败、最近成功/失败和 cooldown，不保存公网 IP、SSID、BSSID、路由器型号或 peer ID。
- ACK/加密路径确认成功会奖励对应 source；probe 超时只会惩罚实际被探测过的 source，避免未发送的 predicted/birthday 候选被误伤。
- predicted 默认每轮 2 个 endpoint；本地历史成功率较高时提高到 4，连续失败时降到 1，进入 cooldown 时降到 0。
- `p2wlan doctor` 输出 `Traversal history` 摘要，便于实机判断 predicted、stun_observed、peer_reflexive、upnp、pcp、nat_pmp、birthday 哪类来源真正有效。

当前已落地的 `NAT-02a` 是家庭路由器端口映射 candidate 初版：

- daemon 在候选采集和 15s refresh 中尝试短超时 UPnP IGD discovery；成功时申请 120s UDP 短租约并把公网映射加入候选，source 标记为 `upnp`。
- 若 UPnP 不可用，会通过默认 IPv4 网关并行尝试 PCP MAP 和 NAT-PMP UDP mapping；成功 candidate 分别标记为 `pcp` / `nat_pmp`。
- 三种协议都优先申请与本地 UDP 相同的外部端口；冲突时接受网关返回的任意有效公网端口。
- 端口映射失败、网关不存在、外部地址非公网或候选数量已达控制面上限时静默跳过，不影响 STUN、prediction 或 relay 主路径。

当前已落地的 `NAT-05c` 是 bounded birthday probing 初版：

- 仅当本机 `NatProfile.birthday_candidate=true` 且 `birthday-probing` 配置开启时，daemon 才围绕远端公网候选生成小窗口 synthetic endpoint。
- 默认每轮最多 4 个 birthday endpoint；历史成功后最多 8 个；连续失败或 cooldown 时为 0，避免形成端口扫描式流量。
...
当前已落地的 `PUNCH-02b` 补齐 UDP 可达与真实数据直连之间的验证窗口：

- 认证 Probe v2 学到的新 peer-reflexive 源端口在 10 秒短窗口内优先于旧的 signaled/succeeded 候选用于直连试探；已确认的 selected pair 仍保持最高优先级。
- 后台重试以匹配的 UDP ACK 作为 probe 成功条件，不再因为 WireGuard 数据尚未返回而立即记为 probe 失败、关闭 trial window。
- 收到认证 peer-reflexive observation 后，daemon 会向该精确源端口发送最多 3 个小型 WireGuard 加密 ICMP 验证包；远端成功解密后才确认 Direct，验证期间 Relay 继续 hedge。
- diagnostics timeline 增加 `encrypted_trial_started`、`encrypted_trial_sent` 和 `retry_probe_succeeded`，用于区分“UDP 已打通”“加密验证已发出”和“真实 Direct 已确认”。
...
- `SEC-03` 定义并实现认证 Probe v2、重放窗口和限速。
  ...
  - 已落地：Ed25519 transcript 签名、控制面当前/设备级 credential 即时撤销入口、relay 本地 ticket `jti` / `device_id` denylist，以及 relay 在线撤销 feed（`jti` / `device_id` / `credential_id`）。
  - 待补齐：独立安全评审、部署面检查和两端新版 daemon 公网 UDP 实机验收；不得据此宣称已达到 UU/WebRTC 级可靠度。
...
- `PATH-02` candidate-pair 状态机。
  - 已落地：candidate pair 记录来源/探测次数/最近探测时间，并按认证 peer-reflexive 与历史质量排序。
...
- `NAT-01` 多观察点 STUN 和行为型 NAT Profile。
  ...
  - `NAT-01b-a` filtering/hairpin/lifetime 诊断字段、prediction/birthday suitability 标记、doctor 建议（已落地）。
  - `NAT-01b-b` 同 socket 主动 filtering/hairpin/lifetime 短探测（已落地）。
  - `NAT-01b-c` 真实 pair 成功率回写、source-level stats、ranking feedback 和 doctor 输出（已落地）。
- `NAT-02` PCP/NAT-PMP/UPnP candidate。
  - `NAT-02a` UPnP IGD / PCP / NAT-PMP UDP 短租约 candidate（已落地）。
- `NAT-03` 本地成功历史和过期策略。
  - `NAT-03a` 本地匿名 traversal history、30 天过期、doctor 输出（已落地）。
- `NAT-04` 自适应 probe 预算。
  - `NAT-04a` predicted candidate 独立 probe budget 与 source 成功率排序（已落地）。
  - `NAT-04b` 基于本地历史动态调整 predicted/birthday budget 与 cooldown（已落地）。
- `NAT-05` 可预测 profile 下的受限端口 prediction。
  - `NAT-05a` 基于稳定 delta 的最多 4 个预测 server-reflexive candidate 广告（已落地）。
  - `NAT-05b` candidate source sidecar、predicted pair stats 和 doctor 输出（已落地）。
  - `NAT-05c` bounded birthday probing 小窗口候选和历史冷却（已落地）。
...
- 完成 Relay token/TLS 设计、Probe v2 skeleton、Ed25519 transcript 签名、本地 ticket denylist 和 relay 在线撤销 feed；继续补安全评审、部署面检查和两端新版 daemon 公网 UDP 实机验收。
...
- candidate-pair 表和状态转换（`PUNCH-02a` 已完成来源标记、排序和 adaptive burst schedule；下一步补 probe budget reason code 与 NAT-01b 行为探测）。
- Relay 与 Direct 并行准备。
