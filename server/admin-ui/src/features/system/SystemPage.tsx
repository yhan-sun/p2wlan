  ...
  return <div className="page-stack">
    ...
    <section className="system-grid">
      <Panel title="进程" subtitle="构建与启动信息">
        <div className="system-hero"><div className="system-hero-icon"><Server size={26} /></div><div><span>运行时间</span><strong>{formatDuration(runtime.data.uptime_seconds)}</strong></div></div>
      ...
      <Panel title="控制面状态" subtitle="不是业务数据面吞吐">
        ...
        <div className="truth-notice system-notice"><CircleAlert size={15} /><span>Control 正常、设备在线、Relay RTT 都不能单独证明真实 TUN 或应用流量已经端到端可达。主机级部署问题使用 p2wlan-server doctor 分层检查。</span></div>
      </Panel>
    </section>
  </div>
}
