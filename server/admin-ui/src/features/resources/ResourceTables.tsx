...
import { StatusPill } from '../../components/ui/console'
import { DataTable, formatAgo, natLabel } from '../../shared/console'
...
export function NetworkTable({ networks }: { networks: AdminNetwork[] }) {
  const columns = useMemo<ColumnDef<AdminNetwork, unknown>[]>(() => [
    ...
    { id: 'type', header: '类型', cell: ({ row }) => <StatusPill tone={row.original.is_room ? 'accent' : 'neutral'}>{row.original.is_room ? '房间网络' : '普通网络'}</StatusPill> },
  ], [])
...
export function RoomTable({ rooms }: { rooms: AdminRoom[] }) {
  const columns = useMemo<ColumnDef<AdminRoom, unknown>[]>(() => [
    ...
    { accessorKey: 'member_count', header: '成员' },
    { id: 'devices', header: '设备', cell: ({ row }) => `${row.original.online_devices}/${row.original.device_count} 在线` },
    { id: 'join', header: '加入', cell: ({ row }) => <StatusPill tone={row.original.join_locked ? 'warning' : 'success'}>{row.original.join_locked ? '已锁定' : '可加入'}</StatusPill> },
  ], [])
