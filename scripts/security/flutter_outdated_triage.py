...
package that is discontinued/retracted. Transitive and SDK-constrained drift is
recorded as evidence so it cannot silently disappear from a release review.
"""
...
def classify(package: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not (outdated or discontinued or retracted):
        return None
...
def run(input_path: Path, head_sha: str | None = None) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "findings": [
                {"code": "invalid_outdated_json", "message": str(error)}
            ],
        }
...
def main() -> int:
    ...
    evidence = run(args.input, args.head_sha)
