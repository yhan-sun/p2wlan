...
void main() {
  ...
  test(
    'auth -> permission -> daemon -> virtualIp -> discover -> done ladder',
    () {
      expect(
        desktop.step(none().copy(hasCredential: true)),
        OnboardingStep.permission,
      );
      expect(
        desktop.step(none().copy(hasCredential: true, permissionGranted: true)),
  ...
  test(
    'mobile and web are remote-management only: onboarding is immediately done',
    () {
  ...
  test(
    'resumability: a fully-progressed state recomputes done without a cursor',
    () {
  ...
  test(
    'resumability: if the daemon died after auth, it resumes at daemon step',
    () {
