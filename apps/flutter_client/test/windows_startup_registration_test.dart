...
void main() {
  ...
  test('stale Windows executable registration is not reported enabled', () async {
    final registration = WindowsStartupRegistration(
      executablePath: executable,
      isSupportedOverride: true,
      processRunner: (_, __) async => ProcessResult(
        0,
        0,
        r'P2WLAN REG_SZ "C:\Old\p2wlan.exe" --p2wlan-login-startup',
        '',
      ),
    );
  ...
  test('writes a quoted application command when enabling login startup', () async {
    final calls = <(String, List<String>)>[];
    final registration = WindowsStartupRegistration(
      executablePath: executable,
      isSupportedOverride: true,
