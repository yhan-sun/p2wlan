...
parts = ''.join(Path(f'.room-connectivity-workbench/patch.{i}').read_text(encoding='utf-8').strip() for i in range(5))
...
amendment_code = amendment.read_text(encoding='utf-8') if amendment.exists() else ''
...
(temp / 'repair-files.json').write_text(json.dumps(files), encoding='utf-8')
