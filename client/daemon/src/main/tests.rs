...
fn overlay_start_gate_requires_overlay_validation() {
    assert!(Cli::try_parse_from(["p2wlan-daemon", "--overlay-start-gate-file", "gate",]).is_err());
...
fn test_clap_parsing() {
    ...
    let conflicting_paths =
        Cli::try_parse_from(["p2wlan-daemon", "--prefer-relay", "--relay-only"]);
...
fn diagnostics_test_config(directory: &std::path::Path) -> Config {
    let mut config =
        Config::generate_default("http://127.0.0.1:8080", "test").expect("default config");
...
fn diagnostics_auth_is_not_touched_when_second_instance_loses_lock() {
    ...
    let first_guard = DiagnosticsAuthGuard::prepare(&mut first_config, &config_path, None)
        .unwrap()
    ...
    assert!(
        !auth_path.exists(),
        "the owner guard removes its session file"
    );
    drop(first_lock);
    let _ = std::fs::remove_dir_all(directory);
}
...
fn diagnostics_auth_guard_cleans_up_after_normal_error_and_unwind_paths() {
    ...
    assert!(
        !path.exists(),
        "error-path guard drop must remove the auth file"
    );
...
async fn diagnostics_auth_guard_repairs_missing_or_stale_file_while_running() {
    ...
    assert!(
        !path.exists(),
        "shutdown must remove the repaired auth file"
    );
    drop(lock);
    let _ = std::fs::remove_dir_all(directory);
}
...
fn one_time_launch_token_file_is_read_once_and_deleted() {
    ...
    assert!(
        !path.exists(),
        "daemon must not retain the launch token file"
    );
    let _ = std::fs::remove_dir_all(directory);
}
...
fn token_transport_cli_has_no_plaintext_token_argument() {
    let parsed = Cli::try_parse_from(["p2wlan-daemon", "--managed", "--token-stdin"])
