use super::{
    DplpmtudPathIdentity, DplpmtudPathIdentitySnapshot, UdpDatagramSize,
    DPLPMTUD_BASE_UDP_DATAGRAM_SIZE, DPLPMTUD_CURRENT_PLPMTU_CONFIRMATION_INTERVAL,
    DPLPMTUD_ERROR_RETRY_INTERVAL, DPLPMTUD_MAX_RETRIES, DPLPMTUD_RAISE_INTERVAL,
    DPLPMTUD_SEARCH_GRANULARITY, MAX_CONSUMED_PROBE_RECEIPTS,
};
...
pub(super) struct OutstandingProbe {
    pub(super) identity: DplpmtudProbeIdentity,
...
pub(super) struct DplpmtudBudgetRevisionState {
    pub(super) path_identity: Option<DplpmtudPathIdentity>,
    pub(super) confirmed_udp_datagram_size: Option<UdpDatagramSize>,
}
...
pub(crate) struct DplpmtudTransition {
    pub(super) decision: DplpmtudTransitionDecision,
    pub(super) next: DplpmtudStateMachine,
}

/// Pure bounded reducer for one exact Direct path.
#[derive(Debug, Clone)]
pub(crate) struct DplpmtudStateMachine {
    pub(super) identity: Option<DplpmtudPathIdentity>,
    pub(super) state: DplpmtudState,
    pub(super) supported: bool,
    /// Conservative starting point.  This is never a usable budget until a
    /// BASE probe is positively ACKed.
    pub(super) base_udp_datagram_size: UdpDatagramSize,
    pub(super) base_confirmed: bool,
    pub(super) confirmed_udp_datagram_size: Option<UdpDatagramSize>,
    pub(super) search_upper_udp_datagram_size: UdpDatagramSize,
    pub(super) pending_candidate_udp_datagram_size: Option<UdpDatagramSize>,
    pub(super) current_plpmtu_confirmation_pending: bool,
    pub(super) current_plpmtu_confirmation_at: Option<Instant>,
    pub(super) outstanding: Option<OutstandingProbe>,
    pub(super) retry_count: u8,
    pub(super) next_sequence: u64,
    pub(super) revision: u64,
    pub(super) probe_count: u64,
    pub(super) success_count: u64,
    pub(super) timeout_count: u64,
    pub(super) send_failure_count: u64,
    pub(super) transient_send_failure_count: u64,
    pub(super) emit_lock_unavailable_count: u64,
    pub(super) session_unavailable_count: u64,
    pub(super) local_packet_too_large_count: u64,
    pub(super) business_packet_too_large_count: u64,
    pub(super) last_send_failure_kind: Option<DplpmtudProbeSendFailure>,
    pub(super) stale_ack_count: u64,
    pub(super) duplicate_ack_count: u64,
    pub(super) last_success_at: Option<Instant>,
    pub(super) last_timeout_at: Option<Instant>,
    pub(super) last_failure_at: Option<Instant>,
    pub(super) raise_at: Option<Instant>,
    pub(super) last_reset_reason: Option<String>,
    pub(super) reset_count: u64,
    pub(super) consumed_receipts: VecDeque<DplpmtudProbeIdentity>,
}
...
pub(super) fn enter_base_error(next: &mut DplpmtudStateMachine, now: Instant) {
    next.base_confirmed = false;
    next.confirmed_udp_datagram_size = None;
    next.pending_candidate_udp_datagram_size = None;
    next.current_plpmtu_confirmation_pending = false;
...
pub(super) fn lower_after_failed_search_candidate(
...
pub(super) fn lower_after_current_confirmation_failure(
    next: &mut DplpmtudStateMachine,
    probe: DplpmtudProbeIdentity,
    _now: Instant,
) {
...
pub(super) fn push_consumed_receipt(
...
pub(super) fn next_search_candidate(
    confirmed: UdpDatagramSize,
