...
pub(super) struct PeerTransportSessions {
...
impl PeerTransportSessions {
    ...
    fn pending_only(token: String, pending: PendingTransportSession) -> Self {
        Self {
            active: None,
            previous: None,
...
impl WireGuardTransport {
    ...
    /// path evidence.
    pub(in crate::transport) async fn session_instance_state(
        &self,
        peer_id: &str,
        session_instance: u64,
    ) -> (bool, bool) {
    ...
    pub(in crate::transport) fn session_instance_state_locked(
    ...
    pub(in crate::transport) fn try_session_instance_state(
        &self,
        peer_id: &str,
        session_instance: u64,
    ) -> Option<(bool, bool)> {
    ...
    /// session in between.
    pub(in crate::transport) async fn session_instance_is_current(
        &self,
        peer_id: &str,
    ...
    pub(in crate::transport) async fn acquire_current_session_evidence_guard(
        &self,
        peer_id: &str,
        session_instance: Option<u64>,
    ...
    pub(in crate::transport) async fn acquire_current_session_evidence_guard_outcome(
        &self,
        peer_id: &str,
        session_instance: Option<u64>,
    ...
    pub(in crate::transport) async fn stage_responder_session_inner(
        &self,
        peer_id: String,
        token: String,
        session: TransportSession,
        ...
        if let Some(existing) = sessions.get_mut(&peer_id) {
            ...
            let mut peer_sessions = PeerTransportSessions::pending_only(
                token.clone(),
                PendingTransportSession {
    ...
    pub(in crate::transport) fn commit_responder_session_in(
        sessions: &mut HashMap<String, PeerTransportSessions>,
        peer_id: &str,
        token: &str,
        now: Instant,
        ...
        } else if let Some(pending) = existing.pending.get_mut(token) {
    ...
    pub(in crate::transport) fn remember_promoted_responder_token(
        &self,
        peer_id: &str,
        token: String,
    ) {
    ...
    pub(in crate::transport) fn pending_promoted_responder_tokens(
        &self,
        peer_id: &str,
    ) -> Vec<String> {
        let now = Instant::now();
...
impl WireGuardTransport {
    ...
    pub(super) async fn expire_previous_session_for_test(&self, peer_id: &str) {
        let mut sessions = self.sessions.lock().await;
        sessions
            .get_mut(peer_id)
            .unwrap()
    ...
    pub(super) async fn last_pending_responder_token_for_test(&self, peer_id: &str) -> String {
        self.sessions
            .lock()
            .await
            .get(peer_id)
    ...
    pub(super) async fn active_responder_token_for_test(&self, peer_id: &str) -> Option<String> {
        self.sessions
            .lock()
            .await
            .get(peer_id)
