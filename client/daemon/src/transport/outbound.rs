use super::{
    debug, global_dataplane_profiler, mpsc, warn, wire_counter, wire_fingerprint, Arc,
    BoundedEmitOutcome, DaemonError, Duration, EncryptedPeerPacket, Future, Instant, Mutex,
    OutboundLossContext, OutboundPacket, OwnedMutexGuard, PeerManager, PendingOutboundPacket,
    Result, SessionBoundEncryption, SessionUnavailableReason, Weak, WireGuardTransport,
    CONTROL_EMIT_LOCK_TIMEOUT, MAX_PENDING_OUTBOUND_PER_PEER, PENDING_OUTBOUND_TTL,
    REASON_SESSION_QUEUE_FULL, REASON_SESSION_QUEUE_REMOVED, REASON_SESSION_QUEUE_STALE,
};
...
impl WireGuardTransport {
    ...
    pub(super) fn outbound_loss_registry(
        &self,
    ...
    pub(super) async fn record_outbound_queue_event(
        &self,
        kind: &str,
        peer_id: &str,
        reason_code: &str,
    ...
    pub(super) async fn outbound_emit_lock(&self, peer_id: &str) -> Arc<Mutex<()>> {
    ...
    pub(super) fn try_outbound_emit_lock(&self, peer_id: &str) -> Option<Arc<Mutex<()>>> {
    ...
    pub(super) async fn outbound_ingress_lock(&self, peer_id: &str) -> Arc<Mutex<()>> {
    ...
    pub(super) async fn remove_idle_outbound_emit_lock(&self, peer_id: &str) {
        let mut locks = self.outbound_emit_locks.lock().await;
        if locks
            .get(peer_id)
    ...
    pub(super) async fn encrypt_outbound_inner(
        &self,
        packet: OutboundPacket,
    ...
    pub(super) async fn queue_pending_outbound(
        &self,
        packet: OutboundPacket,
        reason: &'static str,
    ) {
    ...
    pub(super) async fn queue_pending_outbound_locked(
        &self,
        packet: OutboundPacket,
        reason: &'static str,
    ) {
    ...
    pub(super) async fn forward_raw_outbound(&self, mut packet: OutboundPacket) -> Result<()> {
        let peer_id = packet.peer_id.clone();
        let profiler = global_dataplane_profiler();
        let sampled = packet.trace.as_ref().map(|trace| trace.sampled);
