...
fn test_ticket_verify_valid() {
    ...
    let verifier =
        TicketVerifier::new(keys, DEFAULT_CLOCK_SKEW, "relay-sg-1".into(), "sg".into()).unwrap();
...
fn test_ticket_verify_rejects_wrong_audience() {
    ...
    let verifier =
        TicketVerifier::new(keys, DEFAULT_CLOCK_SKEW, "relay-us-1".into(), "us".into()).unwrap();
...
fn test_ticket_verify_rejects_wrong_region() {
    ...
    let verifier =
        TicketVerifier::new(keys, DEFAULT_CLOCK_SKEW, "relay-sg-1".into(), "us".into()).unwrap();
...
fn test_ticket_verify_rejects_signature_from_another_key() {
    ...
    assert!(
        matches!(
            err,
            RelayError::AuthError(RelayErrorCode::INVALID_TICKET, _)
        ),
...
fn test_ticket_verify_rejects_not_yet_valid_beyond_skew() {
    ...
    assert!(
        matches!(
            err,
            RelayError::AuthError(RelayErrorCode::TICKET_NOT_YET_VALID, _)
        ),
